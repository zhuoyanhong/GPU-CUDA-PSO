...
x_cur_sqr = np.zeros((dim, size))
x_sin = np.zeros((dim, size))
# print("par_cr1 的值： \n", par_cr1[0])
# print("par_cr2 的值： \n", par_cr2[0])
# print('x_cur 的值：\n', x_cur)
# print("v_cur 的值： \n", v_cur)
...
# 适应度函数
def fitness_fun(x_cur_0, x_sin_0, y_cur_0):
    for j in range(size):
        sum = 0
        sum1 = 0
        for i in range(dim):
            x_sin_0[i][j] = np.sin(np.math.sqrt(abs(x_cur_0[i][j])))
            num = x_cur_0[i][j] * x_sin_0[i][j]
            sum += num
            sum1 = a - sum
        y_cur_0[j] = sum1
