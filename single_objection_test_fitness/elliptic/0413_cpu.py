...
# 适应度函数
def fit_coef(x_expo_0, x_coef_0):
    for i in range(dim):
        for j in range(size):
...
def fitness_fun(x_cur_0, x_coef_0, y_cur_0):
    x_sqr_0 = x_cur_0 ** 2
    for j in range(size):
        sum = 0
        for i in range(dim):
...
def pso_cpu(x_cur, v_cur, x_gbest):
    ...
    # 初次计算种群适应度值与全局最优
    fit_coef(x_expo, x_coef)
    fitness_fun(x_cur, x_coef, y_cur)
    y_pbest = y_cur
    ...
    for step in range(iter_num):
        ...
        fitness_fun(x_pre, x_coef, y_pre)
        # 更新个体适应度最优与个体位置最优
