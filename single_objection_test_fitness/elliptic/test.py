...
"""
__device__ void fun_line(float *x_expo, float b)
{
    int tx = blockIdx.x *blockDim.x + threadIdx.x;
    x_expo[tx] = b;
}
__global__ void fun_x_expo(float x_expo[][%(n)s])
{   
    float b;
    for(int line = 0; line < %(m)s; line++)
    {   
        b = line / (%(m)s - 1);
        fun_line(x_expo[line], b);
    }
}
__global__ void fun_coef(float * x_coef, float * x_expo)
{
    int tx = blockIdx.x *blockDim.x + threadIdx.x;
    x_coef[tx] = pow(x_coef[tx], x_expo[tx]);
}

__global__ void fun_multi(float * x_sqr, float * x_coef)
{
    int tx = blockIdx.x *blockDim.x + threadIdx.x;
    x_coef[tx] = x_coef[tx] * x_sqr[tx];
}
__device__ void fun_add_line(float *x, float *y)
{
    int tx = blockIdx.x *blockDim.x + threadIdx.x;
