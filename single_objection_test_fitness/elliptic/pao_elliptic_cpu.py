...
x_coef = a * np.ones((dim, size))
x_expo = np.zeros((dim, size))
# print("par_cr1 的值： \n", par_cr1[0])
# print("par_cr2 的值： \n", par_cr2[0])
# print('x_cur 的值：\n', x_cur)
# print("v_cur 的值： \n", v_cur)
...
def fitness_fun(x_cur_0, x_expo_0, x_coef_0, y_cur_0):
    for i in range(dim):
        for j in range(size):
            x_expo_0[i][j] = i/(dim - 1)
    x_sqr_0 = x_cur_0 ** 2
    x_coef_0 = x_coef_0 ** x_expo_0
    for j in range(size):
        sum = 0
        for i in range(dim):
            sum += x_coef_0[i][j] * x_sqr_0[i][j]
        y_cur_0[j] = sum
...
# print("x_gbest 的值： \n", x_gbest)
def pso_cpu(x_cur, v_cur, x_gbest, y_cur):
    ...
    for step in range(iter_num):
        ...
        fitness_fun(x_cur, x_expo, x_coef, y_pre)
        # 更新个体适应度最优与个体位置最优
    ...
    print('CPU 上计算的 fitness_value_list： ', fitness_value_list)
    print("CPU 上计算的 y_gbest： ", y_gbest)


pso_cpu(x_cur, v_cur, x_gbest, y_cur)
