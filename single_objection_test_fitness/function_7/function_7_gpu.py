...
__global__ void fun_clip_v(float *x, float *max)
{
    int tx = blockIdx.x * blockDim.x + threadIdx.x;
    if(x[tx] > max[tx]) x[tx] = max[tx];
    if(x[tx] < -max[tx]) x[tx] = -max[tx];
}
__global__ void fun_clip_x(float *x, float *max, float *min)
{
    int tx = blockIdx.x * blockDim.x + threadIdx.x;
    if(x[tx] > max[tx]) x[tx] = max[tx];
    if(x[tx] < min[tx]) x[tx] = min[tx];
}
...
fun_clip_v = mod_fun_obj.get_function('fun_clip_v')     # 速度限制
fun_clip_x = mod_fun_obj.get_function('fun_clip_x')     # 位置限制
...
for step in range(iter_num):
    ...
    fun_clip_v(v_cur_gpu, v_max_gpu, grid=grids, block=(n, 1, 1))
    ...
    fun_flush_zeros(x_sin_one_gpu, grid=grids, block=(n, 1, 1))
    fun_flush_zeros(x_sin_two_gpu, grid=grids, block=(n, 1, 1))
    fun_flush_zeros(x_cur_sqr_gpu, grid=grids, block=(n, 1, 1))

    fun_x_update(x_pre_gpu, x_cur_gpu, v_cur_gpu,
                 grid=grids, block=(n, 1, 1))
    fun_clip_x(x_cur_gpu, x_max_gpu, x_min_gpu, grid=grids, block=(n, 1, 1))
...
# print('GPU 计算的 x_pre: \n', x_pre)
# cuda.memcpy_dtoh(x_pbest, x_pbest_gpu)
# print("GPU 计算的 x_pbest: \n", x_pbest)
# cuda.memcpy_dtoh(x_gbest, x_gbest_gpu)
# print("GPU 计算的 x_gbest: \n", x_gbest)
# cuda.memcpy_dtoh(par_cr2, par_cr2_gpu)
# print('GPU 计算的 par_cr2[0]: \n', par_cr2[0])
# cuda.memcpy_dtoh(v_cur, v_cur_gpu)
# print('GPU 计算的 v_cur: \n', v_cur)
# cuda.memcpy_dtoh(x_cur, x_cur_gpu)
# print('GPU 计算的 x_cur: \n', x_cur)
