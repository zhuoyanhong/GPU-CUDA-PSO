...
x_cur = np.random.uniform(x_min, x_max, size=(dim, size))
# print("par_cr1 的值： \n", par_cr1[0])
# print("par_cr2 的值： \n", par_cr2[0])
# print('x_cur 的值：\n', x_cur)
# print("v_cur 的值： \n", v_cur)
# x_sin_one = np.zeros((dim, size))
# x_sin_two = np.zeros((dim, size))
# y_one = np.zeros((size, ))
y_cur = np.zeros((size, ))
y_pre = np.zeros((size, ))
...
def fitness_sin_one(x_cur_0):
    x_sin_one_0 = np.sin(x_cur_0)
    return x_sin_one_0


def fitness_sin_two(x_cur_0):
    x_sin_two_0 = np.sin((x_cur_0 * 0.666667))
    return x_sin_two_0


def fitness_fun(x_cur_0, y_cur_0):
    x_sin_one_0 = fitness_sin_one(x_cur_0)
    x_sin_two_0 = fitness_sin_two(x_cur_0)
    y_one_0 = x_sin_one_0 + x_sin_two_0
    for j in range(size):
        sum = 0
        for i in range(dim):
            sum += y_one_0[i][j]
        y_cur_0[j] = sum + 1.21598 * dim
...
def pso_cpu(x_cur, v_cur, x_gbest):
    ...
    # print('CPU 计算的值 y_cur: \n', y_cur)
    print("CPU 计算的值 min(y_pre): ", min(y_pre))
    print("CPU 计算的值 y_gbest： ", y_gbest)
    # print("CPU 计算的值 x_gbest： \n", x_gbest)
    # print("CPU 计算的值 par_cr1[step]： \n", par_cr1[0])
    # print("CPU 计算的值 par_cr2[step]： \n", par_cr2[0])
    # print('CPU 计算的值 v_cur: \n', v_cur)
    # print('CPU 计算的值 x_cur: \n', x_cur)
