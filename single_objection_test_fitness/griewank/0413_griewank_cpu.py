...
x_sqrt = np.zeros((dim, size))
x_sqr_sum = np.zeros((size, ))
x_mulit = np.ones((size, ))
y_cur = np.zeros((size, ))
y_pre = np.zeros((size, ))
...
# 适应度函数
def fit_sqr(x_cur_0, x_sqr_sum_0):
    for j in range(size):
        sum = 0
        for i in range(dim):
            sum += x_cur_0[i][j] ** 2
...
def fit_mulit(x_cur_0, x_mulit_0, x_sqrt):
    for j in range(size):
        num = 1
        for i in range(dim):
            x_sqrt[i][j] = np.math.sqrt(i + 1)
            num *= np.cos(x_cur_0[i][j]/x_sqrt[i][j])
        x_mulit_0[j] = num


def fitness_fun(x_cur_0, x_sqr_sum_0, x_mulit_0, x_sqrt_0, y_cur):
    fit_sqr(x_cur_0, x_sqr_sum_0)
    fit_mulit(x_cur_0, x_mulit_0, x_sqrt_0)
    for j in range(size):
        y_cur[j] = a * x_sqr_sum_0[j] - x_mulit_0[j] + 1


# fitness_fun(x_cur, x_sqr_sum, x_mulit, y_cur)
...
def pso_cpu(x_cur, v_cur, x_gbest):
    ...
    # 初次计算种群适应度值与全局最优
    fitness_fun(x_cur, x_sqr_sum, x_mulit, x_sqrt, y_cur)
    y_pbest = y_cur
    ...
    for step in range(iter_num):
        ...
        fitness_fun(x_cur, x_sqr_sum, x_mulit, x_sqrt, y_pre)
        # 更新个体适应度最优与个体位置最优
