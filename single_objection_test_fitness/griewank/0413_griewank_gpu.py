...
x_div_cos = np.zeros((m, n), np.float32)
x_div_multi = np.ones((n, ), np.float32)
...
x_div_cos_gpu = cuda.mem_alloc(x_div_cos.nbytes)
...
cuda.memcpy_htod(x_div_cos_gpu, x_div_cos)
...
__global__ void fun_cos(float * x_div, float * x_cos)
{
    int tx = blockIdx.x *blockDim.x + threadIdx.x;
    x_cos[tx] = cos(x_div[tx]);
}
...
__global__ void fun_flush_ones(float *x)
{
    int tx = blockIdx.x *blockDim.x + threadIdx.x;
    x[tx] = 1;
}
...
fun_flush_ones = mod_fun_obj.get_function('fun_flush_ones')
...
fun_cos(x_cur_div_gpu, x_div_cos_gpu, grid=grids, block=(n, 1, 1))
fun_multi(x_div_cos_gpu, x_div_multi_gpu, grid=grids, block=(bn, 1, 1))
...
for step in range(iter_num):
    ...
    fun_flush_zeros(x_cur_div_gpu, grid=grids, block=(n, 1, 1))
    fun_flush_zeros(x_div_cos_gpu, grid=grids, block=(n, 1, 1))
    fun_flush_ones(x_div_multi_gpu, grid=grids, block=(bn, 1, 1))
...
print('GPU 上计算的 min(y_cur)', np.min(y_cur))
